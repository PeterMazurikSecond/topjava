...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal meal : meals) {
            ...
            mapCalories.merge(key, meal.getCalories(), Integer::sum);
        }

        for (UserMeal meal : meals) {
            LocalDate key = meal.getDateTime().toLocalDate();
            boolean excess = TimeUtil.isBetweenHalfOpen(meal.getDateTime().toLocalTime(), startTime, endTime);
            if (excess) {
                if (mapCalories.get(key) > caloriesPerDay) {
